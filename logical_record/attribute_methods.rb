    ...
    module ClassMethods
      ...
      def define_attribute_methods
        unless defined?(@attribute_methods_mutex)
          msg = "It looks like something (probably a gem/plugin) is overriding the " \
                "LogicalRecord::Base.inherited method. It is important that this hook executes so " \
      ...
      def instance_method_already_implemented?(method_name)
        if dangerous_attribute_method?(method_name)
          raise DangerousAttributeError, "#{method_name} is defined by LogicalRecord"
        end
        ...
        if superclass == Base
          ...
          defined && !LogicalRecord::Base.method_defined?(method_name) || super
        end
      end
...
# 
    ...
    # Returns a copy of the attributes hash where all the values have been safely quoted for use
