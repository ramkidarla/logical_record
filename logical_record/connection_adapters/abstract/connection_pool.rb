...
module LogicalRecord
  ...
  module ConnectionAdapters
    ...
    # ways:
    #
    # 1. Simply use LogicalRecord::Base.connection as with Active Record 2.1 and
    #    earlier (pre-connection-pooling). Eventually, when you're done with
    #    the connection(s) and wish it to be returned to the pool, you call
    #    LogicalRecord::Base.clear_active_connections!. This will be the
    #    default behavior for Active Record when used in conjunction with
    #    Action Pack's request handling cycle.
    # 2. Manually check out a connection from the pool with
    #    LogicalRecord::Base.connection_pool.checkout. You are responsible for
    #    returning this connection to the pool when finished by calling
    #    LogicalRecord::Base.connection_pool.checkin(connection).
    # 3. Use LogicalRecord::Base.connection_pool.with_connection(&block), which
...
connection.  For example: LogicalRecord::Base.connection.close
          eowarn
          checkin conn
      ...
      def new_connection
        LogicalRecord::Base.send(spec.adapter_method, spec.config)
      end

      def current_connection_id #:nodoc:
        LogicalRecord::Base.connection_id ||= Thread.current.object_id
      end
    ...
    # LogicalRecord::Base.connection_handler. Active Record models use this to
    ...
    class ConnectionHandler
      ...
      def retrieve_connection_pool(klass)
        ...
        return pool if pool
        return nil if LogicalRecord::Base == klass
    ...
    class ConnectionManagement
      class Proxy # :nodoc:
        ...
        def close
          ...
          LogicalRecord::Base.clear_active_connections! unless testing
        end
      end
